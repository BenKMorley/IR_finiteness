...
def likelihood_maker(n_params, cov_inv, model, res_function):
  """
    ...
    model: A function that has the following callable signature
      ...
      length n_params
    res_function: residual function: See model_definitions.make_res_function
...
def prior_maker(prior_range):
  """
    INPUTS :
    --------
    prior_range: a list of lists, or numpy array, of shape (n_params, 2), where
      the ith component contains the upper and lower bounds of the uniform
      prior of the ith parameter of the fit (floats or ints)
...
def run_pymultinest(prior_range, model, GL_min, GL_max, n_params, directory,
                    ...
                    n_live_points=400, INS=False, clean_files=False,
                    sampling_efficiency=0.3, return_analysis_small=False, tag="",
                    keep_GLmax=True, seed=-1):
  """
    ...
    param_names: list of strings of length n_params. Each entry is the name of
      the corresponding parameter
    prior_name: string, useful for distinguishing results that used different
      priors
    n_live_points: int, number of points active at any time in the MULTINEST
      algorithm. Increasing this increases the accuracy and computational time
      of the algorithm
    INS: Bool, if True multinest will use importance nested sampling method
    clean_files: Bool, if True then the large data files of MULTINEST samples will
      be deleted after completion of the algorithm. This is useful for reducing
      hard-drive load if doing many runs
    sampling_efficiency: See MULTINEST documentation. float between 0 and 1. 0.3
      is used here as this is the reccomended value for Bayesian Evidence
    return_analysis_small: Bool, if True return only key values of the run,
      explicitly... [E, delta_E, sigma_1_range, sigma_2_range, median], where
      E is the log-evidence, delta_E is the error in the log-evidence as estimated
      by MULTINEST (not always accurate), sigma_1_range and sigma_2_range are 
      the 1 and 2 sigma band estimates of parameter values from the posterior
      distribution respectively, median is the median parameter estimate, also from
      the posterior distribution.
    tag: string, change this to label the files associated with a run uniquely
    keep_GLmax: Bool, if True the value of GL_max will be kept in the metadata
      of the filename. You may wish to make this False as MULTINEST has a 100
      charachter limit on filenames, and GL_max isn't particularly important in
